...
export function ReportOverviewPage(_props: { path?: string }) {
    ...
    <ReportLayout path="/report">
      ...
      <div class="grid gap-4 lg:grid-cols-2">
        <Card>
          ...
          {!stats?.projects.length ? (
            <p class="py-6 text-center text-sm text-muted">暂无项目数据</p>
          ) : (
            <PieChart
        ...
        <Card>
          ...
          {!stats?.series.some((s) => s.hours > 0) ? (
            <p class="py-6 text-center text-sm text-muted">暂无数据</p>
          ) : (
            <div class="flex h-36 items-end gap-[3px]">
              {stats.series.map((s) => {
                ...
                return (
                  ...
                  >
                    <div
                      class={`w-full max-w-[14px] rounded-t-sm ${
      ...
      <Card>
        <CardHeader>
          ...
          <Button variant="secondary" size="sm" onClick={() => route('/report/history')}>
            全部 →
          </Button>
        </CardHeader>
        <ul class="divide-y divide-border/70">
          {(stats?.recent || []).slice(0, 5).map((r) => (
            <li key={r.date}>
              <a
                ...
                class="flex items-center justify-between gap-3 py-2.5 text-sm no-underline hover:text-primary"
                onClick={(e) => {
                  e.preventDefault()
          ...
          {!stats?.recent.length ? (
            <li class="py-4 text-sm text-muted">
              尚无归档，去{' '}
              <a
...
function StatTile({
  ...
  return (
    <div
      class={`rounded-2xl border border-border/80 bg-card/90 px-4 py-4 ${
        accent ? 'ring-1 ring-primary/15' : ''
      }`}
    >
      <p class="text-[11px] font-medium uppercase tracking-[0.12em] text-muted">{label}</p>
      <p
        class={`mt-2 font-display text-2xl font-bold tabular-nums ${
          accent ? 'text-primary' : ''
        }`}
      >
        {value}
